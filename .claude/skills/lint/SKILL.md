...
1. Lancer ruff sur le code source :
   - Si `$ARGUMENTS` contient `--fix` : `.venv/bin/ruff check --fix custom_components/`
   - Sinon : `.venv/bin/ruff check custom_components/`
...
- Ne jamais modifier les fichiers `tests/` via ce skill (scope Python : `custom_components/` uniquement)
